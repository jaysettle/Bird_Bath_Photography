...
## Recent Updates (2025-12-12) - Major Codebase Modularization

### 13. **Main.py Modularization** - Reduced from 6,400 to 794 lines (88% reduction)

**Problem**: main.py had grown to 6,400 lines, making it difficult to maintain and navigate.

**Solution**: Extracted classes into focused modules following a phased approach:

**Phase 1 - Configuration & Themes**:
- `src/config_manager.py` (156 lines): ConfigManager class for config loading/saving
- `src/ui/themes.py` (232 lines): apply_dark_theme() function with QPalette and QSS styling

**Phase 2 - Thread Workers**:
- `src/threads/camera_thread.py` (121 lines): QThread for camera operations with USB disconnect handling
- `src/threads/service_monitor.py` (54 lines): QThread for monitoring systemd services
- `src/threads/drive_stats_monitor.py` (60 lines): QThread for Google Drive folder statistics
- `src/threads/gallery_loader.py` (82 lines): QThread for background image loading

**Phase 3 - UI Components**:
- `src/ui/preview_widgets.py` (157 lines): InteractivePreviewLabel for ROI selection
- `src/ui/dialogs/image_viewer.py` (205 lines): ImageViewerDialog for full-size viewing
- `src/ui/logs_tab.py` (140 lines): LogsTab widget for application logs

**Phase 4 - Tab Widgets**:
- `src/ui/services_tab.py` (468 lines): ServicesTab for monitoring services
- `src/ui/config_tab.py` (1,208 lines): ConfigTab for system settings
- `src/ui/gallery_tab.py` (1,239 lines): GalleryTab for viewing captured images
- `src/ui/camera_tab.py` (1,508 lines): CameraTab for camera control and preview
...
├── threads/
│   ├── __init__.py
│   ├── camera_thread.py   # Camera operations thread
│   ├── service_monitor.py # Systemd service monitoring
│   ├── drive_stats_monitor.py
│   └── gallery_loader.py  # Background image loading
└── ui/
    ├── __init__.py
    ├── themes.py          # Dark theme styling
    ├── preview_widgets.py # Interactive preview label
    ├── logs_tab.py
    ├── services_tab.py
    ├── config_tab.py
    ├── gallery_tab.py
...
- Faster IDE loading and code completion
- Independent testing possible
- main.py now only contains MainWindow class

### 14. **Web Interface Improvements** (2025-12-12)

**Navigation Reorganization**:
- Moved tabs to top: Dashboard, Gallery, Species
- Added hamburger menu for settings (Restart App, Clear Cache, etc.)
- Gallery tab now properly switches using inline onclick handlers

**Calendar Enhancement**:
- Added image count badges to each day on the calendar
- New API endpoint: `/api/photo-counts` returns counts by date

**Bug Fixes**:
- Fixed Gallery tab not switching (JavaScript event handler issue)
- Fixed modal close button (X) not working
- Added `/gallery` URL redirect to `/#gallery` hash

**New API Endpoints**:
- `GET /api/photo-counts` - Returns image counts by date for calendar
- `POST /api/clear-thumbnail-cache` - Clears thumbnail cache
- `GET /gallery` - Redirects to main page with gallery tab active
...
## Recent Updates (2025-11-25) - Public Bird Photo Gallery via Tailscale

### 12. **Tailscale Installation & Public Gallery Setup**

**Purpose**: Enable public internet access to bird photo gallery without port forwarding or domain purchase.

**Components Installed**:

| Component | Description |
|-----------|-------------|
| Tailscale | VPN mesh network for secure device connectivity |
| Tailscale Funnel | Public HTTPS proxy exposing local server to internet |
| HTTPS Server | Flask server with SSL on port 8443 |
| Gallery Endpoints | /debug, /live, /api/gallery routes |
...
| Setting | Value |
|---------|-------|
| Tailscale IP | 100.112.148.47 |
| Hostname | raspberry5 |
| Tailnet | tail23d264.ts.net |
| Public URL | https://raspberry5.tail23d264.ts.net |

**New Gallery Endpoints Added to server.py**:

| Endpoint | Purpose |
|----------|---------|
| /live | Real-time gallery with WebSocket updates (has JS issues) |
| /debug | Simplified gallery that works reliably |
| /api/gallery | JSON API returning photo data with species names |

**HTTPS Server Service** (/etc/systemd/system/bird-gallery-https.service):
```ini
[Unit]
...
Type=simple
User=jaysettle
WorkingDirectory=/home/jaysettle/BirdApp/web_interface
ExecStart=/usr/bin/python3 server.py --https --port 8443
Restart=always
...
```bash
# HTTPS Gallery Server
sudo systemctl status bird-gallery-https
sudo systemctl restart bird-gallery-https
sudo systemctl enable bird-gallery-https
sudo journalctl -u bird-gallery-https -f

# Tailscale Funnel
tailscale funnel status
sudo tailscale funnel off                    # Disable public access
sudo tailscale funnel --bg --https=443 https+insecure://localhost:8443  # Re-enable
```

**Public Access URLs**:
| URL | Status | Description |
|-----|--------|-------------|
| https://raspberry5.tail23d264.ts.net/debug | ✅ Working | Simple photo gallery |
| https://raspberry5.tail23d264.ts.net/live | ⚠️ JS issues | Real-time WebSocket gallery |
| https://raspberry5.tail23d264.ts.net/api/gallery | ✅ Working | JSON API |

**How Traffic Flows**:
```
Internet User
    ↓
https://raspberry5.tail23d264.ts.net:443
    ↓
Tailscale Edge Servers (Funnel)
    ↓
Raspberry Pi (100.112.148.47)
    ↓
localhost:8443 (Flask HTTPS server)
    ↓
/home/jaysettle/BirdPhotos/ (photo files)
```

**Files Created/Modified**:
| File | Purpose |
|------|---------|
| /etc/systemd/system/bird-gallery-https.service | HTTPS server systemd unit |
| /home/jaysettle/BirdApp/web_interface/server.py | Added /live, /debug, /api/gallery routes |
| /home/jaysettle/BirdApp/certs/raspberry5.crt | SSL certificate |
| /home/jaysettle/BirdApp/certs/raspberry5.key | SSL private key |

**Gallery Features**:
- **Photo Display**: Shows all photos from ~/BirdPhotos/ organized by date
- **Species Names**: Displays AI-identified bird species on each photo
- **Date Navigation**: Browse by Today, Yesterday, or older dates
- **Lightbox View**: Click photos for full-size viewing
- **Real-time Updates** (on /live): WebSocket pushes new photos as captured
- **Public Access**: No Tailscale installation needed on viewer's device

**Security Notes**:
- Gallery is **publicly accessible** - anyone with the URL can view photos
- No password protection currently enabled
- Consider adding authentication if sharing sensitive content
- Tailscale Funnel can be disabled anytime: `sudo tailscale funnel off`
...
# Test API locally
curl -k https://localhost:8443/api/gallery
...
**Known Issues**:
1. **/live page JavaScript issues**: External CDN resources (Tailwind CSS, Socket.io) may not load properly through Funnel. Use /debug page instead.
2. **Funnel persistence**: Funnel config may not survive reboot. Add to startup script if needed.

**To Add Password Protection** (future enhancement):
```python
# In server.py, add Flask-HTTPAuth
from flask_httpauth import HTTPBasicAuth
auth = HTTPBasicAuth()

@auth.verify_password
def verify_password(username, password):
    return username == 'bird' and password == 'your_password'
